...
** Constructing Generic Algorithms: Principles and Practice
   :SOURCE:
   :Author: Ben Deane
   :Source: https://www.youtube.com/watch?v=InMh3JxbiTs
   :END:

   [Goes through designing an algorithm and how to make it
   generic. Lots of useful points to consider]

   Good algorithms support:
   - 0 length sequences
   - even and odd length sequences
   - various edge cases specific to the algorithm

   Consider iterator categories
   - am I using a standard library algorithm and what iterator does that require?
   - do I look at an element after moving past? If yes it must at
     least be a forward iterator.
   - do I return an element after moving past? If yes must at least be
     a forward iterator.
   - do I need to decrement the iterator? If yes must be at least a
     bidirectional iter.
   - do I need to increment/decrement by more than 1? Then might need
     a random access iterator.
   - Some algorithms have better complexity using stricter iterators.

   Computers can half and double very efficiently (compared to normal
   multiplication and division).

   Test with different types to explore the type requirements.
   ...
   - complexity guarantees:
     ...
     - how many swaps, constructions and conversions?
     - it may vary with iterator category
   - exception behaviour

   The order of parameters for algorithms in the standard library is
   1. Execution Policy
   2. Iterator pair / iterator and length
   3. other input iterators
   4. output iterators
...
** Monoids, Monads, and Applicative Functors: Repeated Software Patterns
   :SOURCE:
   :Author: David Sankel
   :Source: https://www.youtube.com/watch?v=giWCdQ7fnQU
   :END:

   Monoid: a type with a binary operation ⊕ where the operation must
   be associative (a ⊕ b ⊕ c = a ⊕ (b ⊕ c)) and have a special value
   'e' so x ⊕ e = e ⊕ x = x for all values of x. Eg. numbers with plus
   is a monoid, e = 0; numbers with multiplication, e = 1; std::vector
   and concatenation, e = empty vector. 0 or more monoids can be
   squashed together. If there are 0 monoids then just get e.

   =std::optional= of a monoid is a monoid. The operator is applied if
   both operands have a value otherwise return only the one that has a
   value (or a disengaged optional if both are disengaged).

   Monoids scale well and are composable with functions and
   optional. Functions of the same type that return monoids are also
   monoids. They are a highly parallel pattern.

   Functors are like class templates with a single template
   parameter. There is a map function which takes a Functor of type A
   and a function, which takes an A and returns a. Then map returns a
   functor of type B.
   #+BEGIN_SRC cpp
     template <typename A, typename B>
     Functor<B> map(std::function<B(A)>, Functor<A>);
   #+END_SRC
   Giving map the identity function will not change the functor.

   =std::optional= and =std::vector= are functors. =std::pair= can be
   a functor if one template parameter is fixed and the function
   passed to map only modifies the other parameter.

   Applicative Functors are functors with extra operations. 'pure'
   creates a new applicative of type T from a T (ie. wraps a value in
   a container). 'Apply' calls an applicative function on an
   applicative value to get an applicative result.
   #+BEGIN_SRC cpp
     template<typename T>
     optional<T> pure(T t);
   ...
   #+END_SRC
   It can be used to propagate errors in the case of =optional= because
   the null will be the result of any operation where one of the
   operands is null. For =std::vector= apply would take a =vector= of
   functions and a =vector= of values and call every function for
   every value and store the results in a vector. And pure would stick
   a single value into a new =vector=.

   Monads are applicative functors with a join operation. Join takes a
   Monad<Monad<T>> and returns Monad<T>. See [[https://en.m.wikibooks.org/wiki/Haskell/Category_theory][Haskell/Category Theory
   Wikibook]] for monad rules.
   #+BEGIN_SRC cpp
     optional<T> join(optional<optional<T>> a)
     {
         if (a) { return *a; }
         else { return std::nullopt; }
     }
   #+END_SRC
   Monadic bind operations are constructed using pure, apply and
   join. It looks like imperative programming and allows for
   expressing different models of computation in C++.
** Back to Basics: Algebraic Data Types
   :SOURCE:
   :Author: Arthur O'Dwyer
   :Source: https://www.youtube.com/watch?v=OJzmWqCCZaM
   :END:
   Algebraic data types are =std::pair=, =std::tuple=,
   =std::optional= and =std::variant=.

   =std::pair= and =std::tuple= are product types because the number
   of potential states they can be in is the number of states their
   contained types can be in multiplied together (=pair<uint8_t, bool=
   would have 256 * 2 = 512 possible states). The contained types are
   layed out in memory with some padding, similar to a struct. Prefer
   a named struct to using these in a public API. Using them to hold
   reference types will allow the original values to be assigned
   through the =pair/tuple=. =std::tie= can be used like structured
   binding. Use =std::ignore= to ignore parts of the returned
   =pair/tuple= that you don't want to assign to something. =std::tie=
   creates a tuple of references.

   =variant= is a sum type because the number of potential states is
   the alternative types' number of states added together
   (=variant<uint8_t, bool>= would have 256 + 2 = 258 possible
   states). Alternatives occupy the same memory (like a union) with an
   index to indicate, which member is active. Use =get_if<T>= to get a
   pointer to the contained object; it will return =nullptr= if it
   doesn't have a =T=. Can also wrap all possible types into a generic
   lambda and then use =std::visit=. =variant::valueless_by_exception=
   tests if an exception during =emplace= caused no value to be in the
   variant (=emplace= will destruct the held value before constructing
   the new one, if the construction throws an exception the variant
   will end up holding nothing).

   =optional= is a sum type. It's like a =variant= with two types but
   the second type is =nullopt= (which is not saved in memory). It can
   be used for variables with dynamic lifetime without having to use
   the heap. When it has a value it's called "engaged" and
   "disengaged" when it has no value.

   =optional= and =variant= accept =in_place= tag type to make it
   construct in place.

   All the types work with comparison operators. =pair= & =tuple= look
   at all the elements in order. =optional<T>= compares as a =T= with
   the =nullopt= always being less (so disengaged =optional= comes
   before an engaged =optional=).
** Back to Basics: Lambdas from Scratch
   :SOURCE:
   :Author: Arthur O'Dwyer
   :Source: https://www.youtube.com/watch?v=3jCOwajNch0
   :END:
   Only captureless lambdas are convertible to function pointers.
   Unary =+= converts lambdas to a function pointer: =+[](int
   x){return x + 1}= becomes =int( * )(int)= since unary =+= only
   works on scalar types (including function pointers) so an implicit
   conversion is done. This is useful to avoid unnecessary template
   instantiation because otherwise every time a templated function is
   called with a lambda it will create an instance for each lambda
   used (since lambdas have unique types).

   Lambdas are objects. Capture lists add data members.

   In a lambda =this= means whatever it meant in the outer scope that
   contains the lambda.
** atomic<> Weapons: The C++11 Memory Model and Modern Hardware Part 1
   :SOURCE:
   :Author: Herb Sutter
   ...
   If a compiler can prove a modification won't have an observable
   difference then it can do it. In a multi-threaded environment there
   is a lot less that the compiler can prove. Mutexes & atomics tells
   the compiler what happens with shared data because it can't know.
...
** atomic<> Weapons Part 2
   :SOURCE:
   :Author: Herb Sutter :Source:
   http://channel9.msdn.com/Shows/Going+Deep/Cpp-and-Beyond-2012-Herb-Sutter-atomic-Weapons-2-of-2
   :END:

   C/C++ memory model makes it illegal to invent a write to a
   variable. Example where two threads each have a variable each
   protected by a mutex. If both are on same cache line a thread could
   load both variables, modify 1 and then write the whole cache line
   back to memory. It would modify the variable it shouldn't by
   writing it's "original" value, which would cause problems if other
   thread wanted to modify that variable.

   Same problem if both variables are in a struct. The compiler will
   make sure that a variable that is not used isn't touched. Either
   with instructions or padding to make sure variables are on a
   different cache line.

   Not the case for bitfields. Adjacent bitfields count as 1
   "object". Parts of a bitfield might be less than a byte so most
   processors won't be able to load 1 without loading (at least part of)
   the other since most processors won't work on anything less than a
   byte.

   Atomic loads need to be cheap. But can afford to pay more for
   stores since programs load more than they store.

   x86 provides SC atomics quite cheaply. It has a strict memory
   model.

   POWER IA SC loads are expensive (which is half the reason relaxed
   atomics exist in C++).

   ARM v7 needs a memory barrier for SC loads (the other half of the
   reason for relaxed atomics).

   Arm v8 has SC-DRF atomic loads & stores.

   SC-DRF (SC for Data Race Free) is the memory model that software
   has converged on, which means hardware needs to support it. A more
   strict memory model in hardware means leaving performance available
   but a less strict memory model means having to reach for more
   strict features (like full fences) (synchronization) costing
   performance.

   [ *Strongly advises not to use relaxed atomics* ]

   Relaxed atomics are dangerous but can be useful for high
   performance work on ARM v7 & POWER. They aren't SC so can be
   extremely difficult to reason about ("Start to lose
   simultaneously"). They only affect ordering (they are still atomic
   operations).

   =memory_order_*= :
   - =relaxed= : weakest option. Operation can be moved around with no
     ordering guarantees.
   - =acquire=
   - =release=
   - =acq_rel= : like =seq_cst= but not SC. Plain acquire release.
   - =order_consume= : like acquire but more complex. Very hard.
   - =sec_cst= : default. SC.

   Blindly incrementing a counter in launched threads then only
   checking the value after all the threads are joined is example
   where =memory_order_relaxed= is OK.

   Simple flag setting: where worker threads do something until a flag
   is set by the main thread then exit. Accessing the flag from the
   workers can be relaxed because it doesn't matter when the message
   gets there just that it does (and it definitely will). Setting the
   flag from the main thread must be SC (it should be reordered since
   it could end up before the threads are launched or a timer). If the
   threads can set a flag that triggers work on the main thread then
   it should be acquire/release (release on the workers and acquire on
   the main thread) since it publishes data. Doing something and
   reading other state based on a flag indicates a release.

   Generally SC is fine but relaxed atomics could be worth it in a
   tight loop where you want more optimisations (like loop unrolling).

   =volatile= for dealing with things outside the memory model
   (eg. hardware).
...
*** Chapter 3: Patterns
 PIMPL idiom hides impl details by putting them in another class (the
 impl class).  The impl class is forward declared in the header and
 then just a pointer to it is used (opaque pointer).  The problem with
 a default copy constructor is it does a shallow clone so the copied
 objects will have opaque pointers to the same impl object.  You can
 either delete or specify custom copy functions.  In the case that you
 actually want copies to share the impl then use a =shared_ptr= to
 avoid the impl being deleted twice (by the copy and by the original).

 Dependency injection involves passing an object into a class rather
 than having the class create and store the object itself.  This saves
 the class needing to know the details needed to initialize the
 dependency object.  It provides an alternative to a singleton because
 the passed object could be global but it is easier to test because
 the object can be mocked.  The class could be created by a factory
 which maintains a constructed dependency object to pass in.

 If a class has virtual methods always declare the destructor as
 virtual.  If it is not declared virtual and the inheriting class is
 assigned to a base class pointer before being destroyed then the base
 class' destructor will be called (rather than the inheriting class').
*** Chapter 4: Styles
...
** Additional
*** [[https://www.youtube.com/watch?v=AGRWRwi7rD0][Sy Brand - Building an Intuition for Composition]]
    Bit of category theory in C++? One to come back to when you know more.
...
*** [[https://www.youtube.com/watch?v=61DSCW2KL3U][Michael Spertus - “Inference in C++: The big picture”]]
...
*** [[https://www.youtube.com/watch?v=fGuT9zoQ_JA][Russel McClellan - A practical perspective on deep learning in audio software]]
* Maths
...
*** Decorator
 - used to change the responsibilities of an object while maintaining
   the same interface
...
*** Visitor
 ...
 - Works by:
   ...
   - the base class has functions for each of the objects the visitors
     will operate on.
   - those objects have a function that takes a visitor. This function
     calls the relevant function for the object on the passed in
     visitor. The object provides the this pointer.
...
** Additional
   ...
   - [[https://www.youtube.com/watch?v=Gv2I7qTux7g][The Road to Zig 1.0 - Andrew Kelley]]
* IT
** Nix Pills
*** Pill 3
...
** [[https://www.youtube.com/watch?v=uwiaT3MoDVs][Knit, Chisel, Hack: Building Programs in Guile Scheme" - Andy Wingo]]
   Overview of GNU Guile and its features.
** [[https://www.youtube.com/watch?v=eWXoKjWCh60][Building a whole distro on top of a minimalistic language; The story of Gnu Guix]]
** Evolution of Forth
** [[https://stevelosh.com/blog/2021/03/small-common-lisp-cli-programs/][Writing Small CLI Programs in Common Lisp]]                           :lisp:
** [[https://www.youtube.com/watch?v=kMivYV0pEMw&list=WL&index=11][Creating a Common Lisp implementation (Part 3)]]                      :lisp:
   About the process of bootstrapping an CL implementation using an
   existing implementation.
** [[https://www.youtube.com/watch?v=lw6TaiXzHAE&list=WL&index=7&t=115s][Growing a Language]]
**  Four Languages from Forty Years Ago
** [[https://www.youtube.com/watch?v=HB5TrK7A4pI&list=WL&index=6][Gerald Sussman - We Really Don't Know How to Compute]]
